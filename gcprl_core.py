...
def gcprl_enhance(
    ...
    k: float = 1.5,
    window_size: int = 7,
    preserve_diagnostic: bool = True,   # kept for API compat, no longer needed
    local_alpha: float = 0.35,
    stretch: float = 0.95,              # kept for API compat
    ...
    Args:
        ...
        (other args): kept for backward API compatibility, unused in v5.
    ...
    enhanced_u8 = _enhance_gray(image, k, window_size, local_alpha)
...
def gcprl_enhance_color(
    ...
    enhanced_l, meta = gcprl_enhance(
        l_ch, k, window_size, preserve_diagnostic, local_alpha, stretch)
